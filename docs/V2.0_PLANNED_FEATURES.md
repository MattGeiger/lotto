...
- Completed: Personalized homepage (`/new`) now includes an optional top-bar haptics toggle plus semantic client-device haptics for direct controls only: onboarding language selection (`selection`), ticket submit (`medium`), invalid ticket entry (`error`), and back/change-ticket actions (`soft`). Called-ticket celebration remains visual-only on the web path.
- Completed: Personalized homepage (`/new`) now restores tracked-ticket called haptics inside the Capacitor native shell while keeping the browser path visual-only for that async event.
...
- Completed: Shared semantic haptics layer (`HapticsProvider` + `AppHapticIntent`) now backs Arcade and `/new` only, with a persisted `haptics-enabled` preference and dedicated top-bar toggles on eligible surfaces. `/` and `/display` remain haptic-free.
- Completed: Arcade haptics now use semantic meanings for direct user inputs only — `selection` for language choices, accepted Snake turns, and confirmed difficulty changes; `soft` for theme/back toggles; `medium` for primary play/start/pause actions; and `heavy` for reset only.
- Completed: Difficulty sliders now use commit-on-release behavior for haptics so mobile browsers receive one confirmed selection tap instead of drag-driven vibration attempts.
...
- Completed: Added a Capacitor native shell with `@capacitor/haptics` so `/new` ticket-called alerts and Arcade game-loop/ticket-called haptics can run reliably on mobile without changing public-board behavior.
- Completed: Native Arcade haptics now restore async/game-loop semantics: Snake pellet eat uses reward feedback, Snake collision uses failure feedback, Brick brick hits use impact feedback, paddle bounce uses contact feedback, level clear uses reward feedback, and ball loss/game over uses failure feedback.
...
- **WITHIN existing patterns:** Optional client-device haptics remain route-scoped to Arcade and `/new`; public-board routes remain silent on both web and native paths.
...
- Optional client-device haptics are available on Arcade and `/new` only, with explicit per-surface toggles; `/` and `/display` remain haptic-free.
- Native mobile testing validates both direct-input haptics and restored async/game-loop haptics inside the Capacitor shell, while browser validation continues to enforce the web-safe direct-input-only policy.
