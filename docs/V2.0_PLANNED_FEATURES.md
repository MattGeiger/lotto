...
- Completed: Personalized homepage (`/new`) now includes an optional top-bar haptics toggle plus semantic client-device haptics for onboarding language selection (`selection`), ticket submit (`medium`), invalid ticket entry (`error`), back/change-ticket actions (`soft`), and a one-time called-ticket `buzz` aligned with the personalized confetti overlay.
...
- Completed: Arcade haptics now use semantic meanings instead of blanket raw presets — `selection` for language choices and accepted Snake turns, `soft` for theme/back toggles, `medium` for primary play/start/pause actions, `heavy` for reset only, `success` for Snake pellet collection and Brick level clear, `rigid` for brick destruction, `light` for paddle bounce, `error` for gameplay failures, and `buzz` for tracked ticket-called alerts.
- Completed: Device tuning pass for mobile reliability — weaker short-duration library taps were replaced with stronger app-owned patterns for selection ticks, gameplay contact/impact/reward feedback, and the tracked ticket-called alert.
