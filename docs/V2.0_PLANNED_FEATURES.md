...
- Completed: Haptic feedback via `web-haptics` across all arcade interactions — `buzz` on ticket-called event, `error` on brick destruction and game over, `light` on paddle bounce, `success` on Snake pellet eaten, and `heavy` on all arcade button presses. Scoped exclusively to arcade routes. Engine exposes `paddleBounced` in `TickResult` for page-level haptic detection.
