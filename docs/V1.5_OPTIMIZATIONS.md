...
**Status:** Phases 1, 2, 3, 4a, and key Phase 4/6 items are implemented. Remaining work reprioritized below.  
**Target branch:** `main`  
...
- On iPad mini 4 (A8, 2 GB RAM, iPadOS 15.8.6 security branch), `/admin` can still show lag:
  - Typing latency in range inputs remains noticeable in worst cases, but is now less severe after recent optimizations.
  - On-device testing after the latest pass reports remaining lag around 1-2 seconds in worst typing paths.
  - Tap latency from snapshot-history coupling is reduced after decoupling refresh from the action critical path.
...
### Phase 3 Follow-up (P0): Measure Residual iPad Lag After Decoupling
- Capture fresh iPad mini 4 traces focused on typing latency after decoupling rollout.
- Confirm tap-path p95 stays stable under slow snapshot-list responses.
- Use findings to guide next optimization focus (remaining field isolation vs. payload/hydration work).
...
- On-device validation update:
  - iPad mini 4 typing/input lag is improved but still reproducible in worst cases.
- `npm test -- tests/admin-page-actions.test.tsx tests/admin-range-locking.test.tsx`:
  - 14 assertions passed (including slow-snapshot decoupling regression coverage).
- `npm test`:
  - 378 assertions passed across 40 test files.
