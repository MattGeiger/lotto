...
- History option rendering is now capped (`SNAPSHOT_RENDER_PAGE_SIZE`) with progressive expansion via "Show older snapshots."
...
- **Phase 11.2 (done):** Deferred draw-path snapshot refresh and capped History snapshot option rendering with progressive loading.
...
8. Load History with >200 snapshots and verify initial render caps options while "Show older snapshots" progressively expands choices.
...
- Implementation update:
  ...
  - Draw-triggered snapshot refresh is deferred and History option rendering is now capped with progressive expansion to reduce iPad layout pressure.
