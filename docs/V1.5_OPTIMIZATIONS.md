...
**Status:** Phases 1, 2, and 4a implemented in v1.5.1. Remaining work reprioritized below.  
**Target branch:** `main`  
...
### 2C. `/admin` Still Re-renders as a Monolith on Keystrokes
- `AdminPage` still owns all local form state in one client component (`src/app/admin/page.tsx:100` onward).
- Range field keystrokes update parent state directly (`src/app/admin/page.tsx:874`, `src/app/admin/page.tsx:903`), triggering full-page reconciliation.

Result: input cost includes unrelated UI sections (history, QR, operating hours, live state cards).

### 2D. `previewUndrawnCount` Remains O(range) for End-Range Edits
- `previewUndrawnCount` loops `ticket = startNumber .. previewEnd` and checks `drawnSet` each time (`src/app/admin/page.tsx:580`, `src/app/admin/page.tsx:596`).

Result: this scales with ticket range size and can still surface in slower edit scenarios.

### 2E. Async Error Propagation Can Leave Unhandled Rejections
- `sendAction` rethrows after toast (`src/app/admin/page.tsx:247`).
- Callers like `handleNextServing` and `handlePrevServing` do not catch (`src/app/admin/page.tsx:667`, `src/app/admin/page.tsx:672`).
- Current test checkpoint still reports a Vitest unhandled rejection warning even with passing assertions.
...
### Phase 4 (P1): Component Splitting for Keystroke Isolation
- Extract input-heavy sections into isolated child components with local form state.
- Use `React.memo` and stable callbacks to reduce rerender fan-out.
- Keep public behavior unchanged.

### Phase 5 (P1): Async Error Hardening
- Ensure action handlers catch rethrown async errors where needed.
- Keep toast behavior but avoid unhandled rejections in tests/runtime.

### Phase 6 (P2): Reduce O(range) Preview Cost
- Add an O(1) path for append preview where possible:
  - `additionalUndrawn = max(0, previewEnd - state.endNumber)`
  - `previewUndrawn = serverUndrawnCount + additionalUndrawn` when preconditions hold.
- Keep range-scan fallback only for mixed/edge cases that truly require it.
...
| **P1** | Split admin input sections to isolate rerenders | Planned | High | Medium |
| **P1** | Fix async action error propagation (no unhandled rejections) | Planned | High | Low |
| **P2** | O(1) preview-undrawn path for append-range edits | Planned | Medium-High | Low |
...
- `npm test -- tests/admin-page-actions.test.tsx tests/admin-memoization.test.ts`:
  - 29 assertions passed.
  - 1 unhandled rejection warning remains (known issue tied to async error propagation path).
- `npm run build`:
  - Build passed.
...
### Planned Code Touch Points (Next Implementation Phase)
- `src/app/admin/page.tsx`
- `src/components/admin/*` (new split sections)
