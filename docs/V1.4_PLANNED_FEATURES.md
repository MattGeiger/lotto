...
- Calibrated text morph timing to the accepted baseline after live review:
  - "Now Serving": `spring(80, 16, 0.45)`
  - `LanguageMorphText`: `spring(90, 16, 0.4)`
- Trialed a slower +50% pass and rolled it back in-session because it reduced perceived responsiveness on the public display.
- Lint/test/build remain green after integration.

### Phase 6: Theme Switcher View Transition Parity (Completed - February 13, 2026)

- Reviewed Animate UI `ThemeToggler` reference implementation and API (`direction`, immediate icon update, View Transition fallback behavior).
- Added local primitive parity component at `src/components/animate-ui/primitives/effects/theme-toggler.tsx`.
- Updated `src/components/theme-switcher.tsx` to use `ThemeToggler` for Light/Dark/System transitions while preserving existing `Hi-viz` contrast mode and dropdown structure.
- Added reduced-motion guarding for the View Transition path so accessibility preferences still force immediate updates.
- Added test coverage in `tests/theme-switcher.test.tsx` for `document.startViewTransition` usage.
- Full suite + production build validated after integration (125 tests passing).
